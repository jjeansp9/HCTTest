...
//    }
    ...
    private fun setAlbumPictureUI(holder: VH, position: Int){
        ...
        Log.i("albumUrl", items[position].albumUploadPicture)
        Glide.with(context).load(items[position].albumUploadPicture).into(holder.binding.imgAlbumUploadPictureFirst)
        // 등록된 프로필이 없을 때
//        when {
